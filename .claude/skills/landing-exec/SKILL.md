...
- **報告前に env / フラグの実在を確認しない**。存在しない環境変数を設定しても何も
  起きず、実行は既定値のまま通る——「掃引した」と書いた 3 回が全部同じ設定だった実例
  がある（`RIVUS_CHUNK_SIZE` は存在せず、chunk size は CLI の `--chunk-size`）。
  使う前に `grep -rhoE '"RIVUS_[A-Z0-9_]+"' crates/` と `--help` で実在を確かめ、
  効いたことを出力（strategy 行・WPROF・件数の変化）で裏取りしてから書く。
