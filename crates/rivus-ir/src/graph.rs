...
pub enum Discovery {
    ...
    /// delimited records — a feed that never ends. Like `Watch` it is outside
    /// the deterministic-op set (§0.14): arrival order is environmental.
    /// Parse / `to_source` are always-std; evaluation requires the off-by-default
    /// `net` feature (a feature-less run refuses pre-run, never-silent). Unlike
    /// `Watch` (which needs the `unbounded`/`notify` feature) this rides the
    /// `net` feature — the two are gated apart (see [`PlanGraph::uses_net`] /
    /// [`PlanGraph::uses_watch`]).
    Subscribe(String),
}
...
impl Discovery {
    ...
    /// `to_source` and the parallel-read size gate (`Glob`/`Watch`/`Subscribe`
    /// never plan a byte-range read — their codec is `Discover`/network).
...
pub fn is_http_url(p: &str) -> bool {
    let l = p.trim_start();
    // ASCII case-insensitive scheme check without allocating.
            ...
            Op::Source {
                ...
                match codec {
                    Codec::Csv {
                        ...
                        // endpoint and the `subscribe` verb; a plain file/HTTP
                        // `open` keeps the bare path. Both share the modifiers.
                        // An `http://` URL is quoted so it re-lexes as one string
                        // token (reversible, §33); a plain path stays bare.
                    ...
                    // JSONL `subscribe` renders `subscribe "tcp://…" as json`;
                    // a file/HTTP `open` keeps the bare path (extension picks the
                    // codec). `as json` makes the network codec explicit (an
                    // endpoint has no extension to infer from).
...
impl PlanGraph {
    ...
    /// Does the plan contain the unbounded **file-`watch`** source (§28.12)? This
    /// is the one gated by the `unbounded` / `notify` feature — distinct from the
    /// network `subscribe` (gated by `net`, see [`Self::uses_net`]). The runtime
    /// refuses a `watch` plan built without `unbounded` pre-run (never-silent).
    ...
    /// Does the plan reach the **network** (§33) — an `http://` / `https://`
    /// source, or the unbounded `subscribe` stream? Gated by the `net` feature; a
    /// feature-less plan that touches the network refuses pre-run (never-silent,
    /// the `regex`/`gzip`/`unbounded` shape). Independent of `uses_watch` so the
    /// two off-by-default transports are gated apart.
