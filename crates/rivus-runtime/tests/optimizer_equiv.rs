...
fn colon_chain_is_byte_equivalent_to_parenthesized_forms() {
    ...
    // ProjectExpr items as the surviving parenthesized spelling (`(name) as
    // alias` — the `(name:type) as alias` cast form was retired by the
    // design/38 flip), so the runtime output must be identical row-for-row —
    ...
    let paren = format!("F:\n open {p}\n |> amount :amt :decimal(2) (note) as memo id\n;");
