...
/// `with filename` already attaches the same handle (it is `with source` plus a
/// materialized column, slice 2-②b), so `source.uri` works under it too.
#[test]
fn with_filename_also_attaches_handle() {
    let f = TempCsv(gendata::write_temp_bytes(
        "prov_filename",
        b"id,name\n1,a\n2,b\n",
    ));
    ...
    assert!(
        src.iter().all(|s| *s == p),
        "with filename must attach the origin handle too, got {src:?}"
    );
}
