...
impl SourceJsonl {
    ...
    /// its body with the single-pass streaming JSONL reader (a network body can't
    /// be re-read for the two-pass inference). A capability denial or a
    /// transport/HTTP error is fatal (the source has no data).
    #[cfg(feature = "net")]
    fn load_http(&mut self, ctx: &mut OpCtx) {
        let reader = match crate::net::http_get(&self.path) {
            Ok(r) => r,
