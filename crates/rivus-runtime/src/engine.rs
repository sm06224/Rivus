...
fn build_ops(
    ...
    graph
        ...
        .map(|node| {
            ...
            } else if mem_id.is_some() && matches!(node.op, Op::SinkCsv { .. }) {
                operators::collector()
            } else {
...
fn flush_parallel_sinks(graph: &PlanGraph, res: &mut RunResult) {
    ...
    for out in std::mem::take(&mut res.outputs) {
        if let Op::SinkCsv { path } = &graph.nodes[out.node_id].op {
            if let Err(e) = operators::write_csv_file(path, &out.chunks) {
                res.errors.push(
                    ErrorEvent::new(
                        Severity::Critical,
                        ErrorScope::Graph,
...
fn merge_results(
    ...
    for (node_id, chunks) in by_node {
        if let Op::SinkCsv { path } = &graph.nodes[node_id].op {
            if let Err(e) = operators::write_csv_file(path, &chunks) {
                errors.push(
                    ErrorEvent::new(
                        Severity::Critical,
                        ErrorScope::Graph,
