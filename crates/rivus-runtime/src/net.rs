...
//! `std::net`.
//!
//! Two transports, both producing a `Box<dyn BufRead + Send>` the existing
//! codecs decode unchanged (the orthogonality of §28.2 — only the transport
//! layer changes):
...
//!   newline-delimited records (an **unbounded** source, §33.4).
//!
...
/// `subscribe "tcp://host:port"` (§33): dial the feed and return its byte stream
/// as a `BufRead` the codec line-streams. Unbounded — the stream ends when the
/// peer closes (or the engine saturates a `take N` downstream).
...
mod tests {
    ...
    fn parses_tcp_urls() {
        ...
        assert!(parse_tcp_url("tcp://127.0.0.1").is_err()); // no port
    }
