...
/// block holding the Mermaid DAG and the canonical (optimized) flow. The block
/// is regenerated from the IR each time and never parsed back — the Mermaid
/// fence is inert and the flow is an inert ` ```text ` block (not an executable
/// ` ```flow `, so `run` ignores it) — so editing inside it is overwritten. Ends
/// with exactly one newline for idempotent upsert.
fn generated_region(graph: &rivus_ir::PlanGraph) -> String {
    ...
    s.push_str("```\n");
    // The canonical (optimized) flow as inert display text (§31.4-3): a `text`
    // fence, never an executable `flow` fence, so it is not run and round-trips
    // as prose. Regenerated from the IR, so it stays in lock-step with the DAG.
    s.push_str("```text\n");
    s.push_str(graph.to_source().trim_end_matches('\n'));
    s.push('\n');
    s.push_str("```\n");
    s.push_str(GEN_END);
    s.push('\n');
