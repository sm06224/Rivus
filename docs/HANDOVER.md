...
（提案タグ v1.3.0-dev.15 はカット待ち）。squash 後は
`git fetch origin main && git reset --hard origin/main` して継続。

> **現フォーカス＝§29 surface 収束（s1-s3 landed/レビュー待ち → TZ 批准 → s4）。**
...
- **裁定**：§29.5 ②③⑤⑥＝**issue #137 で批准済**（範囲形 `@start..end`・単位型族自動・
  重なり許容/網羅不要・可変長範囲外・`char[N]` padding=値・TZ「ありあり」・regex 評価のみ
  feature gate）。**TZ named zone の方式のみ issue #140 で再批准待ち**（推奨＝(a) std-only
  略称テーブルを s3・(b) tzdata は将来 feature-gated スライス）。
...
- **s3 非TZ＝PR #141（レビュー待ち）**：検証つき `ddd` 曜日（日付と照合・矛盾=カウント付き
  失敗）・`[ja-jp]` ロケール（書式文字列先頭タグ・std-only 表・IR 変更ゼロ）・`n…n` サブ秒
  （最長 run が unit を導出 1-3→ms/4-6→µs/7-9→ns・`DtSpec.subsec` で #93 strip 回避・
  Display は unit>Sec で全幅小数）。**同梱修正**：lexer `lex_string` の byte→char 化け
  （全マルチバイト文字列リテラル破壊）・`DateTime::format` 同型化け・`parse_with_format`
  のマルチバイトリテラル中 char 境界 panic（byte スライス化）。AUTO_FORMATS 不変更。
- **次**：#140 裁可 → TZ (a) 実装（s3 続き）。その後 **s4**＝`~`/`'…'` regex リテラル/`$_[i]`/
  `|!` 複数検証＋`{}` サブフロー（parse/to_source 常時 std・評価のみ off-by-default `regex`
  feature・off=never-silent＝既存 `Func::Regexp` と同構成）。
