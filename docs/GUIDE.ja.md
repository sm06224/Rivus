...
実在の `""` は引用付き `""` で round-trip します。`null` は集約で skip され
（`sum`/`avg`/`min`/`max` は無視）、算術を伝播します（`null + x → null`）。

> **過渡的挙動（v1.3.0-dev・意図的）。** null モデルは段階導入中です。**集約は既に
> `null` を skip** します（`sum`/`avg`/`min`/`max`）が、**フィルタ/比較（`|?`）と
> `dropna`/`fill` は数値 `null` をまだ backing `0` として扱います** — つまり
> `|? age == 0` は現状空欄にも一致し、`|? age > 5` は空欄を除外します。述語の
> null 対応（`null` → 残さない、SQL 流）と null 対応 `dropna`/`fill` は次段
> （design 26 STEP 2-②）で入ります。列全体が `null` の集約 skip だけ意図的に先行
> 実装しています。それまでは欠損でフィルタしたい場合 `:str` 列で `== ""` を使って
> ください。
