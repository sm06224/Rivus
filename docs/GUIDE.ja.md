...
### ネットワーキング — `open "http://…"` と `subscribe "tcp://…"`

Rivus は同じフロー語彙のまま**ネットワーク越し**に読み込めます — 変わるのは
**トランスポート**だけです（設計 §33）。いずれも**クライアント側のみ**（Rivus は
リスナーを bind しません）で、`--features net` の裏に隔離されます（std のみ・
**依存追加ゼロ**＝`std::net` 上の最小 HTTP/1.1 クライアント）。パースと
`rivus explain` は常に動作し、既定ビルドでのネットワーク実行は再ビルド誘導つきで
実行前に明示拒否されます（黙った誤答なし）。

**`open "http://host[:port]/path"` — 有界 HTTP GET。** リモートの CSV/JSON を
ローカルファイルと全く同じように扱えます（filter pushdown も効く）：

```
rivus run --features net -c '
Adults:
  open "http://127.0.0.1:8080/data.csv"   # CSV。.jsonl / `as json` で JSON
  |? age >= 18, country == "JP"
  |> name age country
  print
;'
...
- ボディ枠は `Content-Length` / `Transfer-Encoding: chunked` / 接続クローズの3形を
  扱い、`3xx` リダイレクトを最大5回追従します。`https://` は**非対応**（TLS/証明書
  運用は範囲外・§28.12.5）— 保護チャネル（loopback、将来の WireGuard/QUIC）を使います。
- 有界で、**ストリーミング・有界メモリ**で decode します（ボディは単一パス＝
  ネットワークストリームはシーク不可なので serial・圧縮ファイルと同様）。結果は
  **chunk-size 非依存**です。

**`subscribe "tcp://host:port"` — 非有界 TCP フィード。** 生成側に dial し、行区切り
レコードを購読します（既定 CSV・`as json` で JSON Lines）：
...
Live:
  ...
  |? age >= 18
  |> name age
  take 100        # 有界化。または peer クローズで終了
  print
;'
```

- `watch` と同様に**非有界**（§0.14）：optimizer と並列実行は手を付けず、下流の
  全ストリーム集約は拒否されます（窓が必要＝後続スライス）。パススルー（filter /
  project / take）は流れます。
- **ロスレス背圧**：消費が遅いと OS の TCP 受信ウィンドウが詰まり生成側が自然に
  待ちます — 取りこぼしはありません。

**capability 境界**（両トランスポート・§28.12.4/5）：**loopback**
（`127.0.0.1` / `::1` / `localhost`）は常に到達可。それ以外のホストは
`RIVUS_CAP_NET_HOSTS`（カンマ区切りの `host` または `host:port`）に列挙された時のみ
到達でき、無ければ**拒否**されます（メッセージは対象のみを示し allowlist は漏らさない）。
資格情報はプラン・テレメトリ・エラーに載りません。読み取りタイムアウトは
`RIVUS_NET_TIMEOUT_MS`（既定 30 秒）で調整します。

自己完結の end-to-end デモ（loopback の HTTP サーバ＋TCP フィード）は
[`examples/networking-demo.sh`](../examples/networking-demo.sh) にあります。
...
この領域は**データセット中心の系譜**の Mermaid DAG と、正準化（最適化済み）フローを
inert な ` ```text ` ブロックとして保持します。DAG では**ノード＝データセット**——源
（🗄️）・名前付きスコープ/中間（📦）・沈（📄）——で、各々に列と型を表示します（静的スキーマ
伝播パス・§32.1）。**辺＝操作**（🔍 filter・🔗 join・📋 project・📊 group・🔀 sort・
🏆 take）。スキーマを変える操作（group/join/project）は独立のステップ（独立の辺＋結果
ボックス）になり、畳まれるのはスキーマ不変の行操作（filter/sort/take/distinct）だけです。
列はソーススキーマを宣言したとき
（`open f.csv (a:int …)`）に出ます——未宣言ソースは列なしのボックス（静的にスキーマが
分からないため）。毎回 IR から再生成され（決して読み戻さない）、中を編集しても上書き
