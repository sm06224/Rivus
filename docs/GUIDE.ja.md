...
- **不正日付で never-silent**：`2024-02-30` のような存在しない日付（その他不正
  セル）は既定値 `1970-01-01` として保持（continue-first）**かつ** その損失を
  エラーストリームに報告 — `N value(s) in column 'day' (as date) could not be
  parsed; kept as default 0`。**空** セルは「欠測」であって失敗ではない
...
- **`HH:mm:ss` のみ**（時 `0..23`、分/秒 `0..59`）。`25:00:00` のような不正時刻は
  `00:00:00` として保持（continue-first）**かつ** エラーストリームに surface
  （`N value(s) in column '…' (as time) could not be parsed`）。空セルは「欠測」で
  カウントされない。ゼロ埋めなし入力（`9:5:0`）もパースして `HH:mm:ss` に
...
（tee）fan-out** はすべて忠実に往復します。fmt は **round-trip に正直**で、自身の
出力を再パースして検証し、まだ無損失に描けない一部のまれな fan-out 形を含む
場合は、別物に書き換えず非ゼロ終了でソースを変更せず拒否します。
