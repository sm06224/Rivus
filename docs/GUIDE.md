...
```mermaid
flowchart TD
  n0[("open<br/>examples/users.csv")]:::src
  n1["filter<br/>age &gt;= 20"]
  n2["group<br/>country"]
  n0 --> n1
  n1 --> n2
...
The region holds the Mermaid DAG (nodes shaped by role — source = cylinder,
sink = parallelogram, join = hexagon — and tinted via `classDef`) and the
canonical, optimized flow as an inert ` ```text ` block. It is regenerated from
the IR each time (never parsed back), so editing inside it is overwritten and
re-running is idempotent — hand-written prose around it is preserved. Both
fences are inert (the flow is `text`, not an executable `flow`), so the region
never affects what `run` executes.
