...
### 25.2b 先頭パイプ省略・`group` 別名（入力 sugar・#171 確定 2026-06）

転送 4 種は**語頭の `|` を任意**にし、`|? |> |# |!` と**裸 `|`** を併存させる。書き手は
流れを `|` で素直につなげる（単一行チェーンも可）。これは**入力 sugar に限る**：消費した
`|` は IR に保存せず、`to_source` は常に**型付きパイプの正準形**（複数行・`|?`/`|>`/`|#`/
`|!`）で清書する（**二重正準形を作らない**）。

```
F: open a.csv | where age >= 30 | sort score desc | save out.csv ;   # 入力
# ↓ to_source（正準形）
F:
  open a.csv
  |? $_.age >= 30
...
- **裸 `|` がキーワード・ステージ直前のときだけ**消費する（`| where`/`| sort`/`| group` …）。
  `| name` は名前付きフロー適用（§25.4）のまま、`|> …` は projection のまま、いずれも不変。
- **body 冒頭の `|`**（ソース前）は parse error 維持（流れの起点はソース）。
- `group KEY… func:col…` は `|#` の**読みやすい別名**。`to_source` は `|#` を出す（`group` は
  予約語化。実列名 `group` は `item("group")` で退避）。`sort`/`distinct`/`explode` 等と同列の
  既存トレードオフ。
- §31 Literate の ```` ```flow ```` フェンス内でも同一（フェンス本体は通常フロー構文）。
  #162/#163 の §31 文法と衝突しない（stage-1 zero-semantic-change を維持）。

### 25.2a Verb 命名ポリシー（恒久・統括確定 2026-06-07）

**Verb のみ。`Verb-Noun` は採らない。** Rivus はデータフロー特化＝対象は常にフローなので
Noun は暗黙。

- **正名**は PowerShell の**動詞/別名の語彙**を流用する（ハイフン複合 `Verb-Noun` は不採用）。
- **短縮は alias**（例：`?`/`%`、`ls` の `gci`/`dir`）。既存 `open`/`save`/`sort`/`|?` 等は
  据え置き（= alias 扱い）。alias は parser で正名へ解決し、`to_source` は**正名**を出す。
- **エスケープハッチ**：1 つの verb が別対象に分岐して曖昧になる時だけ、別 verb か Noun 修飾を
  足す。
- 「PowerShell 正名を canonical に」「ハイフン verb 許可」案は**撤回**（ハイフン字句解析は不要）。
