...
- **§27.4 との関係**: 動的ファイル名は「1 ストリーム→1 パス（データ依存）」、分割出力は
  「1 ストリーム→キー別 N パス」。前者は後者の退化形として実装してもよい（**裁定済 #143 ①（2026-06-11）：退化形として統合**＝正準形 `save TEMPLATE [by KEY…] [as flat]`・プレースホルダ＝キー・テンプレート外 `by` キーはエラー）。
...
- **意味論**: パーティションキーで行を振り分け、キー別に別ファイルへ。**パイプラインブレーカ**
  ...
  並列でも byte-identical（書き分けは順序非依存キー＝決定的）。高基数ガード＝**修正裁定
  #143 ③（2026-06-11）：予防的 Fatal 上限なし・オプトイン要求として全パーティションを書き切る・
  silent fallback 禁止・資源圧は LRU/spill 等の工学で吸収・書込不能のみパーティション単位
  Recoverable surface（他継続）**。
- **null キー**: パーティションキーが null の行 → DuckDB 同様 `key=__HIVE_DEFAULT_PARTITION__`
  相当の「null パーティション」へ（§26 と整合：null は単一グループに畳む）。**裁定済 #143 ②
  （2026-06-11）：DuckDB 互換センチネル採用・全レイアウト同一文字列・キー値は `%` 自身を含む
  決定的パーセントエスケープ（key→path 単射）**。
