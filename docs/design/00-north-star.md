...
制御プレーンはテレメトリ→制御だけでなく、**フローの入力/対象を引数として注入**できる
（読む Resource 集合・glob・パラメータを外から束縛）。実装は §25.3 の値ホール `$x` ＋束縛
機構の発展形に乗る（`ls $pattern` / `read $resources` を制御プレーンが束縛）。discovery/read
は供給元を問わない（§28.3）。
