...
> 残り — **B2 `quic` ＝ won't-do で close（§35 批准・#211）**：`quic` は **opt-in feature の
> まま温存・非出荷・`full` 非搭載**（下記 §33.4）／
...
## 33.4 トランスポートの dep-zero 戦略（§35 批准反映・#211）

§35（分散トランスポートの dep-zero 戦略）の批准により、以下を本設計の正式な立場とする：

- **保護チャネルの dep-zero 主経路は kernel WireGuard**（#149-2 のとおり・Rivus に暗号
  コード/依存ゼロ）。
- **非 WireGuard 環境の安全境界は §34.4 sidecar/Transport Service 終端**：Rivus は平文で
  UDS/localhost に話し、**暗号（TLS/QUIC/mTLS/mesh）は sidecar が終端**する。「将来 QUIC が
  要る」場合の答えは quinn+ring の vendor ではなく **QUIC 終端 sidecar**。
  なお **sidecar の批准は設計上の位置づけ**であり、**UDS 実装（§34.2/§34.4）は従来どおり
  staged（批准制・順次着地）のまま**——本追補が実装スケジュールを繰り上げるものではない。
- **QUIC バックエンド（feature `quic`）は opt-in であり配布（`full`）には含めない**（非出荷）。
  「WireGuard も sidecar も使えず、ring 依存を受容する」ユーザ向けの spike として温存する。
  §33.1(B) と「MVP / 次」の `full` 収載記述はこの裁定で**上書き**される（B2 won't-do・§35.4）。
...
- **次**：§17.3 stage 分割＋shuffle（DAG を複数ワーカへ）／Arrow IPC shuffle／コーディネータの
  telemetry 集約（§17.7）／checkpoint/replay（§17.8）。（旧記載の「`quic` を `full` 収載」は
  **§35 批准で won't-do**＝§33.4。「§34 チャネル↔QUIC ストリーム 1:1」は opt-in `quic` 内の
  話であり非出荷。）
