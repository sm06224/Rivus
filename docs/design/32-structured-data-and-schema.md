...
1. **構造化データ**：JSON/Parquet のネスト（オブジェクト・配列）を、現状の degrade-to-string
   （`crates/rivus-runtime/src/jsonl.rs:35` `JVal::Raw(String)`・ネストは Str レーンへ退避）から
...
- `ProjectExpr`（`operators/transform.rs:1235` impl）：出力型＝**評価した式のレーン**（`col.dtype()`）。
...
optimizer は全スキーマを作らず、式を**構造的に走査**して列名を集める（`optimizer/lib.rs:472`
...
`Schema`（`crates/rivus-core/src/schema.rs:26`＝`Vec<Field>`・`Field{name, DataType}`・`index_of`）
...
**構造化 ≠ 行指向ボックス**。現状 core はフラット・スカラ列のみ（`crates/rivus-core/src/chunk.rs:300`
`ColumnData` の Bool/I64/F64/Dec/DateTime/Duration/Date/Time/Str/Resource＝10レーン・**Struct/List
無し**）。`DataType`（`value.rs:1401`）・`Value`（`value.rs:1322`）も同様。ネストは degrade-to-string。
...
- `GroupBy{keys: Vec<String>}`（行724）・`Sort{keys: Vec<(String,bool)>}`（行684）・
  `Distinct{keys: Vec<String>}`（行689）・`Join{left_keys, right_keys: Vec<String>}`（行744）・
  `FilterProject{fields: Option<Vec<String>>}`（行732）。
...
（`Access::Fast`=`$_.f`/`Deep`=`$_..f`/`Source`=`source.uri`・行231）・`Expr::FieldAt(u32)`=`$_[i]`・
...
- **`[i]`（List 添字）**：0始まり。範囲外は **null ＋計上**（既存 `$_[i]`＝`FieldAt` の
  「範囲外→null＋counted」規則・`expr.rs:241` と整合）。
