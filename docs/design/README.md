...
| 33 | [networking-execution](33-networking-execution.md) | 実装（feature `net` 本命＝std/依存ゼロ・`quic` 代替） | **保護チャネル分散実行（ピラー3）**＝#149 裁定附記に忠実。**IR を配備成果物として遠隔ワーカで実行し結果を byte-identical に返す**（`rivus serve`／`run --on`）。**素のリスナー無し**（wg-iface/loopback バインド＋peer allowlist）・**暗号はカーネル WireGuard へ委譲（埋め込まない）**・credit bounded pull。完動テスト済。QUIC 代替（`quic`・quinn/rustls/rcgen）＝相互認証/静的鍵 pin/byte-identical ストリーム**完動**（`serve --quic`／`--on quic://`）。loopback 例外層＝`open http://`／`subscribe tcp://`（CSV+JSON）。 |
| 34 | [transport-architecture](34-transport-architecture.md) | 一部実装（チャネル分離＋イベント可観測性＝landed／CPU 予算・ホスト共有 Service・DPU＝批准待ち） | **トランスポート層の CPU 予算化**（統括意見具申）。中心命題＝通信は速くするより**CPU 消費を制御**（暗号 SIMD vs Rivus SIMD 競合）。PMCN の**通信責務集約**＋QUIC の**チャネル論理分離**。**landed**：1 接続上の Control/Data/Telemetry チャネル分離（フレーム先頭チャネルバイト）＋**イベント中心可観測性**（`flow.started`/`flow.completed`/`transfer.done`・`run_remote_observed`・CLI `--on` の stderr イベント）。**批准待ち**：CPU affinity 予算（feature `cpubudget`・Linux）／**ホスト共有 Transport Service**（UDS/SHM・複数 Rivus 同居の通信集約・セッション共有・Telemetry 集約）／DPU/SmartNIC オフロード（Service 差し替え）。byte-identity 不変。 |
