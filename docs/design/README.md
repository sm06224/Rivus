...
| 43 | [rolling-windows](43-rolling-windows.md) | 批准済み・実装済み | **移動集計（#63・Track C 残り最後の大物）**＝`rolling_sum/avg/min/max(col, N) over …` を P3 窓 item 族へ（新キーワードなし・N 必須・端点 null・直列）。核心規則＝**f64 は滑り累算禁止**（毎行再計算で値を窓内容の純関数に＝byte-identity 構成的）・exact レーン（i64/decimal/duration）は滑り可（bit 一致 oracle で pin）・min/max は単調 deque。`rolling_std`/`min_periods`/str は保留 |
| 44 | [resample-gapfill](44-resample-gapfill.md) | 提案（裁可待ち） | **欠損時間バケットの行生成（#62・Track C 最終）**＝新 verb `resample KEY "STEP" [over …]` が観測 min..max の STEP 格子上の欠損点へ null 行を生成（Rivus 初の row-creating op・§30.4「真に新しい意味論」圏）。**埋めは既存 `fill` との合成**（新機構なし）。出力は `(over…, KEY)` 昇順の sorted 契約・blocking・直列。格子外/null KEY は保持＋計数 surface・格子爆発は Fatal ガード。from/to 明示境界・range ソースは保留 |
