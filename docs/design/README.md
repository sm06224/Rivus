...
| 33 | [networking-execution](33-networking-execution.md) | 実装（feature `net`・std-only・依存ゼロ） | **ネットワーキング実行**＝§28.10 slice 5 後続の socket/http を §28.12.5 に忠実に着地。**クライアント側のみ**（リスナーは bind しない）の2トランスポート：`open "http://…"`（有界 GET・std 最小 HTTP/1.1・CSV+JSON・content-length/chunked/close・redirect・単一パス・chunk-size 非依存）／`subscribe "tcp://…"`（非有界 client feed・CSV+JSON・ロスレス背圧・`watch` と同じ非有界/決定性タグ）。capability＝loopback 既定＋`RIVUS_CAP_NET_HOSTS` allowlist（拒否は対象のみ・秘匿でなく境界）。`https`/TLS は範囲外（証明書運用）。parse/explain 常時 std・評価のみ `net` ゲート（never-silent 拒否）。次：HTTP POST sink（出力鏡像 §28.7）／`read` の `http://`／将来 WireGuard/QUIC。 |
