...
| 33 | [networking-execution](33-networking-execution.md) | 実装（feature `net` 本命＝std/依存ゼロ・`quic` 代替） | **保護チャネル分散実行（ピラー3）**＝#149 裁定附記に忠実。**IR を配備成果物として遠隔ワーカで実行し結果を byte-identical に返す**（`rivus serve`／`run --on`）。**素のリスナー無し**（wg-iface/loopback バインド＋peer allowlist）・**暗号はカーネル WireGuard へ委譲（埋め込まない）**・credit bounded pull。完動テスト済。QUIC 代替（`quic`・quinn/rustls/rcgen）＝相互認証/静的鍵 pin 完動・ストリーム WIP。loopback 例外層＝`open http://`／`subscribe tcp://`（CSV+JSON）。 |
