...
| 43 | [rolling-windows](43-rolling-windows.md) | 提案（裁可待ち） | **移動集計（#63・Track C 残り最後の大物）**＝`rolling_sum/avg/min/max(col, N) over …` を P3 窓 item 族へ（新キーワードなし・N 必須・端点 null・直列）。核心規則＝**f64 は滑り累算禁止**（毎行再計算で値を窓内容の純関数に＝byte-identity 構成的）・exact レーン（i64/decimal/duration）は滑り可（bit 一致 oracle で pin）・min/max は単調 deque。`rolling_std`/`min_periods`/str は保留 |
