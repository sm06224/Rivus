...
parse 可＋fmt 正典化・次リリースでエラー化。**エラー化の前提条件**: asof join
に同一文内チェーンが続く形は fmt が明示拒否する既知の to_source 連鎖描画の穴
（#233 由来・P4 の退行ではない）があり、その修正スライスの着地が旧綴り
エラー化リリースの**ブロッカー** — 穴が残ったまま flip すると該当ユーザーの
移行手段が消えるため）。P5 は使用調査待ちで保留。
