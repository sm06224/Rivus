...
- スライス案：**s1 UDS フロント＝プレ実装 landed**（`rivus serve --uds PATH`／`--on uds://PATH`・
  `distributed::{serve_uds, run_remote_uds}`）→ s2 セッション共有 → s3 ルーティング/集約。
...
- **landed**：§34.1 論理チャネル分離（Control/Data/Telemetry）＋§34.2 イベント中心可観測性
  ...
  `distributed_emits_telemetry_events`）＋**§34.4 s1 UDS フロント（プレ実装）**
  （`serve_uds`/`run_remote_uds`・CLI `serve --uds`／`--on uds://`・transport 非依存プロトコル
  抽出・test `distributed_uds_transport_service_round_trips`）。byte-identity 不変。
- **次（批准後）**：§34.3 CPU 予算/affinity（feature `cpubudget`・Linux）→ §34.4 s2 セッション
  共有/集約 → QUIC backend をチャネルに 1:1 マップ。
