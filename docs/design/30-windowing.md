...
> **2026-07-25 追記（#157 と design/38 P3 の整合）**: #157 が却下した `over` は
> SQL の **式レベル OVER 全文法**（ORDER BY／frame 指定つき窓式）。design/38 P3 が
> 導入した `over` は**パーティションのみ**の窓 item 句（`(lag(x,1) over u)` —
> 順序はソース順・frame 無し）で、#157 の「窓＝派生グループ化キー」原則の適用範囲
> （tumbling 集約窓）とは別物。両裁定は両立しており、本書の却下記述を P3 の反例と
> 読まないこと。
