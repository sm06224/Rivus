...
| ✅ | **Missing-value imputation** (欠測補完) | `dropna [cols]` ✅, `fill col VALUE` ✅, `fill col ffill\|bfill` ✅ (directional carry across chunks), **`fill col mean\|median`** ✅ (whole-column statistic over the non-empty numeric cells). All chunk-size independent; bfill/mean/median are pipeline-breakers. **Null model (#81):** the reader now reads a blank/unparseable cell — **numeric lanes included** — as a first-class `null` (no longer `0`), and aggregations skip it (STEP 2-① landed). `dropna`/`fill` and filter predicates are null-aware across every lane (STEP 2-② landed): BUG-A is fixed and `dropna_drops_blank_numeric_rows_bug_a` is green; group-by/distinct fold null keys; sort orders nulls last. |
