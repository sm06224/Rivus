...
| 🚧 | **DuckDB parity oracle** (migration track, theme 1) | an external test oracle: run the same null-semantics queries in Rivus and DuckDB and assert the **output row counts agree** — the real migration blocker is that null handling must not change counts. The `duckdb` CLI is shelled out **only when present** (skipped otherwise), so the **zero-dependency default is preserved** (`tests/duckdb_parity.rs`). Cases: filter null, inner-join null-key, group-by null-key, and the **maintainer's real ETL (scaled)** — `yyMMddHHmmss` ts + 34-char id + dirty numeric (blank / `***.**` / garbage `0-.-2` / a broken row), extract `substr(id,22,4)=='0059'`; a value that won't parse must **not** drop the row (Rivus keeps it, val→null). CI runs it on a **non-blocking** `duckdb-parity` lane (DuckDB CLI pinned & version-checked, `continue-on-error`) so a divergence shows without reddening the gate. Next: add more real ETL cases as supplied; pin the DuckDB CLI download by sha256 (resilience vs asset replacement — deferred: needs the published checksum). |
