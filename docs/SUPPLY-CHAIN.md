...
| **Parquet** ✅ *integrated (read slice)* | [`parquet`](https://crates.io/crates/parquet) (apache/arrow-rs) — **without the `arrow` feature** | `parquet` | official Apache project, the standard, actively released, Apache-2.0. **Adopted read-only**: `default-features = false, features = ["snap", "flate2", "flate2-rust_backened"]` — the row/column API only, which shrinks the tree from 60+ crates (arrow) to ~22, with **no C build step anywhere**. Codecs: uncompressed + snappy (`snap` v1, BSD-3-Clause, the de-facto writer default) + gzip through the **same pure-Rust `flate2`/`miniz_oxide` backend the `gzip` feature already vets** (feature-unified, no second copy); zstd/lz4/brotli codecs deliberately **not** enabled (the `zstd` codec would pull C `zstd-sys`). Runtime transitive tree (all pure-Rust / FFI-decl only, permissive): `ahash`→`cfg-if`+`getrandom`(→`libc`)+`once_cell`+`zerocopy`, `bytes`, `chrono`→`iana-time-zone`+`num-traits`(→`libm`), `half`, `hashbrown`, `num-bigint`→`num-integer`, `paste`/`seq-macro`/`zerocopy-derive` (proc-macros), `snap`, `twox-hash` (already vetted via `ruzstd`). Behind the source-operator boundary; a feature-less run **refuses the plan pre-run** (never-silent); nested columns are a Fatal-with-guidance (a later slice). Write side and predicate/projection pushdown into row-group metadata are follow-ups. **Not promoted into `full`** — the release-build set is the maintainer's distribution call. |
